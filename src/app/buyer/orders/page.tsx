...
export default async function BuyerOrdersPage() {
    ...
    const orders = await Order.find({ buyer: dbUser._id })
        .sort({ createdAt: -1 })
        .populate('product', 'title price images status')
        .populate('seller', 'email sellerName firstName lastName')
        .lean();
    ...
    return (
        <div className="space-y-10 pb-20">
            <div className="relative pt-6">
                ...
                <p className="text-slate-600 text-sm font-medium tracking-wide mt-1">Track your purchases and their status.</p>
            </div>
            ...
            {orders.length === 0 ? (
                <div className="glass p-20 rounded-[2rem] border-white/[0.04] text-center">
                    ...
                    <h3 className="text-2xl font-bold text-white mb-2 tracking-tight">No Orders Yet</h3>
                    <p className="text-slate-600 font-light">Browse the marketplace and make your first purchase!</p>
                </div>
            ) : (
                            ...
                            <div className="p-6 flex flex-col sm:flex-row gap-6">
                                ...
                                <div className="flex-grow space-y-2">
                                    ...
                                    <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm text-slate-500">
                                        <span>Seller: {order.seller?.sellerName || `${order.seller?.firstName || ''} ${order.seller?.lastName || ''}`.trim() || order.seller?.email}</span>
